...
class BOOMV3:
	...
	def __init__(self, cfg, device=None):
		...
		self.optim = torch.optim.Adam(
			[
				...
				{"params": self.model._Qps.parameters()},
				{
	...
	def _estimate_value(self, z, actions, task, horizon, eval_mode=False):
		...
		return G + discount * self.model.Qp(
			z, self.model.pi(z, task)[1], task, return_type="avg"
		)
		# return G + discount * self.model.V(
		# 	z, task, return_type="avg"
		# )
	...
	@torch.no_grad()
	def _td_target_qp(self, next_z, reward, task):
		"""
		高效版 Best-of-N TD Target 计算（适配 next_z: [H, B, z_dim]）
		优化点：减少冗余维度变换、复用 Q 网络输入、内存连续化、避免重复计算
		"""
		N = 16
		H, B, z_dim = next_z.shape
		device = next_z.device
		dtype = next_z.dtype

		# -------------------------- 优化1：减少冗余维度变换（直接计算展平维度） --------------------------
		# 预计算展平维度，避免多次 reshape/unsqueeze 嵌套
		flat_dim = H * B * N
		# 方式1：用 expand 替代 repeat（浅拷贝，节省内存+时间）
		next_z_expanded = next_z.unsqueeze(2).expand(H, B, N, z_dim)  # [H,B,N,z_dim]
		# 方式2：直接生成展平索引，避免中间大张量（更省内存）
		next_z_reshaped = next_z_expanded.reshape(flat_dim, z_dim)  # [HBN, z_dim]

		# -------------------------- 优化3：批量生成动作（复用 pi 输出，避免冗余计算） --------------------------
		# 直接调用 pi，一次性生成所有动作，无中间变量
		_, pi_batch, _, _ = self.model.pi(next_z_reshaped, task)
		# action_dim = pi_batch.shape[-1]

		# -------------------------- 优化4：Q 值计算（内存连续化 + 避免重复reshape） --------------------------
		# 强制张量连续（contiguous），加速后续 reshape/gather（GPU 对连续张量更友好）
		pi_batch = pi_batch.contiguous()
		...
		# -------------------------- 优化5：最优动作选择（向量化计算，无循环） --------------------------
		# 1. 重塑 Q 值时直接指定维度，避免多次 reshape
		q_values_reshaped = q_values.view(H, B, N)  # 去掉最后一维 1，减少维度操作 [H,B,N]
		# 2. 同时计算 argmax 和 max Q（避免后续重复调用 Q 网络）
		best_q_vals, best_q_indices = torch.max(q_values_reshaped, dim=2, keepdim=True)  # [H,B,1]

		# # 3. Gather 最优动作（优化索引生成，减少 expand 次数）
		# # 直接生成 [H,B,1,action_dim] 的索引，避免分步扩展
		# best_indices = best_q_indices.unsqueeze(-1).expand(H, B, 1, action_dim)
		# pi_samples = pi_batch.view(H, B, N, action_dim)  # [H,B,N,action_dim]
		# pi_best = pi_samples.gather(dim=2, index=best_indices).squeeze(2)  # [H,B,action_dim]
		
		discount = (
			self.discount[task].unsqueeze(-1) if self.cfg.multitask else self.discount
		)
		td_target = reward + discount * best_q_vals  # [H,B,1]
	...
	def update(self, replay_sample, step):
		...
		with torch.no_grad():
			...
			td_targets_qp = self._td_target_qp(next_z, reward, task)
		...
		qps = self.model.Qp(_zs, action, task, return_type="all")
		reward_preds = self.model.reward(_zs, action, task)

		# Compute losses
		reward_loss, q_value_loss, qp_value_loss = 0, 0, 0
		for t in range(self.cfg.horizon):
			reward_loss += (
			...
			for qp in range(self.cfg.num_q):
				qp_value_loss += (
					math.soft_ce(qps[qp][t], td_targets_qp[t], self.cfg).mean()
					* self.cfg.rho**t
				)
		consistency_loss *= 1 / self.cfg.horizon
		reward_loss *= 1 / self.cfg.horizon
		q_value_loss *= 1 / (self.cfg.horizon * self.cfg.num_q)
		qp_value_loss *= 1 / (self.cfg.horizon * self.cfg.num_q)

		total_loss = (
			self.cfg.consistency_coef * consistency_loss
			+ self.cfg.reward_coef * reward_loss
			+ self.cfg.value_coef * q_value_loss
			+ self.cfg.value_coef * qp_value_loss
		)
		...
		self.model.soft_update_target_Qp()
		...
		return {
			...
			"qp_value_loss": float(qp_value_loss.mean().item()),
			"pi_loss": pi_loss,
			"pi_loss_q": pi_loss_q,
