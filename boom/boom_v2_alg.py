...
class BOOMV2:
	...
	@torch.no_grad()
	def _td_target_v(self, z, task):
		"""
		Compute the TD-target from a reward and the observation at the following time step for value function.
		Args:
				z (torch.Tensor): Latent state at the following time step.
				reward (torch.Tensor): Reward at the current time step.
				task (torch.Tensor): Task index (only used for multi-task experiments).
		Returns:
				torch.Tensor: TD-target for value function.
		"""
		pi = self.model.pi(z, task)[1]
		next_z = self.model.next(z, pi, task)
		pre_reward = math.two_hot_inv(self.model.reward(z, pi, task), self.cfg)
		discount = (
			self.discount[task].unsqueeze(-1) if self.cfg.multitask else self.discount
		)
		return pre_reward + discount * self.model.V(
			next_z, task, return_type="min", target=True
		)
	...
	def update(self, replay_sample, step):
		...
		with torch.no_grad():
			...
			td_targets_v = self._td_target_v(z, task)
		...
		vs = self.model.V(_zs, task, return_type="all")
		reward_preds = self.model.reward(_zs, action, task)

		# Compute losses
		reward_loss, q_value_loss, v_value_loss = 0, 0, 0
		for t in range(self.cfg.horizon):
			reward_loss += (
			...
			for v in range(self.cfg.num_v):
				v_value_loss += (
					math.soft_ce(vs[v][t], td_targets_v[t], self.cfg).mean()
					* self.cfg.rho**t
				)
		consistency_loss *= 1 / self.cfg.horizon
		reward_loss *= 1 / self.cfg.horizon
		q_value_loss *= 1 / (self.cfg.horizon * self.cfg.num_q)
		v_value_loss *= 1 / (self.cfg.horizon * self.cfg.num_v)
