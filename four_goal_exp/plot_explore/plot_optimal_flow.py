...
def plot_four_goal_trajectories_clean(
    ...
    # Draw directional arrows for all four goals
    arrow_length = 0.8
    ...
    for goal_name, goal_info in GOAL_CONFIG.items():
        ...
        arrow = FancyArrow(start_x, start_y, end_x - start_x, end_y - start_y,
                    width=0.12, head_width=0.35, head_length=0.35,
                    color=goal_info['color'], alpha=0.5, zorder=8,
    ...
    # Plot the optimal (shortest) trajectory for each goal
    for goal_name, goal_info in GOAL_CONFIG.items():
    ...
    # No longer plot unsuccessful trajectories - showing only optimal path
...
def generate_trajectories(
    ...
    for i in range(num_trajectories):
        ...
        # Distribute trajectories evenly among all four goals
        goal_idx = i % 4
        goal_names_list = list(GOAL_CONFIG.keys())
        target_goal = goal_names_list[goal_idx]

        # Primary direction towards the target goal with random variation
        primary_angle = np.random.normal(GOAL_CONFIG[target_goal]['optimal_angle'], 0.4)
...
def main():
    ...
    output_path = "figures/explore/optimal_flow.png"
